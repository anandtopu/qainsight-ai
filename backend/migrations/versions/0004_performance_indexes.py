...
def upgrade() -> None:
    ...
    op.create_index("ix_test_cases_run_duration", "test_cases", ["test_run_id", "duration_ms"])

    # ── test_runs ───────────────────────────────────────────────────────────
    # Project + start_time for ordered run history (most common dashboard query)
    op.create_index("ix_test_runs_project_start", "test_runs", ["project_id", "start_time"])
    # Build number lookups for idempotent ingestion upserts
    op.create_index("ix_test_runs_project_build", "test_runs", ["project_id", "build_number"])

    # ── test_case_history ───────────────────────────────────────────────────
    # Baseline lookups for anomaly detection (fingerprint + run ordering)
    op.create_index("ix_history_fingerprint_run", "test_case_history", ["test_fingerprint", "test_run_id"])

    # ── ai_analyses ─────────────────────────────────────────────────────────
    # Run-level AI analysis lookups
    op.create_index("ix_ai_analyses_run", "ai_analyses", ["test_run_id"])
    ...
    op.create_index("ix_stage_results_pipeline", "agent_stage_results", ["pipeline_run_id"])
    ...
    # ── defects ──────────────────────────────────────────────────────────────
    # Run-level defect lookups for triage agent
    op.create_index("ix_defects_run", "defects", ["test_run_id"])


def downgrade() -> None:
    op.drop_index("ix_defects_run", table_name="defects")
    ...
    op.drop_index("ix_ai_analyses_run", table_name="ai_analyses")
