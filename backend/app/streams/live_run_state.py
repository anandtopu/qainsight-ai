...
class RedisLiveRunState:
    ...
    async def start(cls, run_id: str, project_id: str, build_number: str, total_tests: int = 0) -> None:
        ...
        await redis.sadd(LIVE_ACTIVE_SET, run_id)
    ...
    async def record_test_event(
        ...
        if counter_field:
            await redis.hincrby(key, counter_field, 1)
        ...
        if total_tests > 0:
            ...
            current_total = int(await redis.hget(key, "total") or 0)
    ...
    async def get(cls, run_id: str) -> Optional[dict]:
        ...
        redis = get_redis()
        raw = await redis.hgetall(_STATE_KEY(run_id))
        if not raw:
            return None
    ...
    async def get_all_active(cls) -> list[dict]:
        ...
        redis = get_redis()
        run_ids = await redis.smembers(LIVE_ACTIVE_SET)
        states = []
        dead = []
        ...
        if dead:
            await redis.srem(LIVE_ACTIVE_SET, *dead)
        return states
    ...
    async def complete(cls, run_id: str) -> Optional[dict]:
        ...
        await redis.srem(LIVE_ACTIVE_SET, run_id)
    ...
    def _deserialise(raw: dict) -> dict:
        ...
        result["pass_rate"] = round((result.get("passed", 0) / completed * 100), 2) if completed else 0.0
        return result
