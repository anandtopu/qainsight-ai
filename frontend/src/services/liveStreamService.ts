...
const liveStreamService = {
  createSession: (payload: LiveSessionCreate) =>
    api.post<LiveSessionResponse>('/api/v1/stream/sessions', payload).then(r => r.data),

  getActiveSessions: (projectId?: string) => {
    const params = projectId ? { project_id: projectId } : {}
    return api
      .get<ActiveSessionsResponse>('/api/v1/stream/active', { params })
      .then(r => r.data)
  },

  getSession: (sessionId: string) =>
    api.get<SessionDetail>(`/api/v1/stream/sessions/${sessionId}`).then(r => r.data),

  closeSession: (sessionId: string) =>
    api.delete(`/api/v1/stream/sessions/${sessionId}`),
}
